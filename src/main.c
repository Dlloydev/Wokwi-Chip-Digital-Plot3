...
void chip_timer_event(void *user_data) {
  ...
  chip->sample_d0 = pin_read(chip->pin_D0);
  if (chip->sample_d0 < chip->sample_min0) chip->sample_min0 = chip->sample_d0;
  if (chip->sample_d0 > chip->sample_max0) chip->sample_max0 = chip->sample_d0;
  chip->sample_d1 = pin_read(chip->pin_D1);
  if (chip->sample_d1 < chip->sample_min1) chip->sample_min1 = chip->sample_d1;
  if (chip->sample_d1 > chip->sample_max1) chip->sample_max1 = chip->sample_d1;
  chip->sample_d2 = pin_read(chip->pin_D2);
  if (chip->sample_d2 < chip->sample_min2) chip->sample_min2 = chip->sample_d2;
  if (chip->sample_d2 > chip->sample_max2) chip->sample_max2 = chip->sample_d2;

  if (attr_read(chip->trigger_attr) == 2) { // falling
    if (chip->plot_x < chip->fb_w - 1) draw_plot(chip);
    if ((chip->plot_x == chip->fb_w - 1) && (chip->sample_d0 < chip->sample_last_d0)) draw_plot(chip);
  }
